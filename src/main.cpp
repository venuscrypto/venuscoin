...
#if defined(NDEBUG)
# error "Venuscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3f3e8d3ec6ce9feee653e3e8c30f15544d4abe47e8106c1ffc05a59a4ee23a77");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Venuscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Venuscoin Signed Message:\n";
...
{
    // Venuscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Venuscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Venuscoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Venuscoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Venuscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Venuscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3f3e8d3ec6ce9feee653e3e8c30f15544d4abe47e8106c1ffc05a59a4ee23a77");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 09/Dec/2017 Jerusalem row: Clashes erupt over Trump move";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047084710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1512844730;
        ...
        assert(block.hashMerkleRoot == uint256("0xaf10b852403ee2f08dc9053689b8d37bd47ee26f237e9ca83c7e2c19ecb2855a"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xf0, 0xf6, 0xfb }; // Venuscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VenuscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VenuscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VenuscoinMiner(CWallet *pwallet)
{
    printf("VenuscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("venuscoin-miner");
    ...
    try { loop {
        ...
        printf("Running VenuscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VenuscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VenuscoinMiner, pwallet));
}
